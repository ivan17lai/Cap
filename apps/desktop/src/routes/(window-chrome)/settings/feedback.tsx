	...
	return (
		<div class="cap-settings-page flex flex-col w-full h-full custom-scroll">
			<SettingsPageContent>
				<Section
					title={t("Feedback")}
					description={t("Help us improve Cap by submitting feedback or reporting bugs. We'll get right on it.")}
				>
					<form
						class="space-y-4"
						onSubmit={(e) => {
					...
					>
						<fieldset disabled={submission.pending}>
							...
							{submission.result?.success && (
								<p class="text-sm text-primary">{t("Thank you for your feedback!")}</p>
							)}
				...
				<Section
					...
					description={t("Have questions, want to share ideas, or just hang out? Join the Cap Discord community.")}
				>
					<Button
				...
				<Section
					...
					description={t("Upload your logs to help us diagnose issues with Cap. No personal information is included.")}
				>
					<Button
							...
							return (
								<div class="space-y-3 text-sm">
									<Show when={osVersion}>
										{(ver) => (
											<div class="space-y-1">
												<p class="text-gray-11 font-medium">{t("Operating System")}</p>
									...
									<div class="space-y-1">
										<p class="text-gray-11 font-medium">{t("Capture Support")}</p>
									...
									<Show when={(d.availableEncoders as string[])?.length > 0}>
										<div class="space-y-1">
											<p class="text-gray-11 font-medium">{t("Available Encoders")}</p>
