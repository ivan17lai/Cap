			...
			<SettingsPageContent>
				...
				<ServerURLSetting
					...
					onChange={async (v) => {
						...
						if (
							!(await confirm(
								t("Are you sure you want to change the server URL to '{origin}'? You will need to sign in again.").replace("{origin}", origin),
							))
						)
							return;
...
function LanguageSection() {
	const { locale, setLocale, displayMode, setDisplayMode } = useI18n();

	// Second language options — add more locales here in the future
	const secondLocaleOptions: Array<{ value: Locale; label: string }> = [
	...
	const modeOptions: Array<{
	...
	}> = [
		{
			value: "mixed",
			label: "混合 Mixed",
			desc: "English (中文) — primary with second language in parentheses",
		},
		{ value: "pure", label: "純 Pure", desc: "Only show the second language" },
	];

	return (
		<Section
			title="Language / 語言"
			description="Choose your second language for the interface."
		>
			<SectionCard padded>
				<div class="flex flex-col gap-4">
					<SettingItem
						label="Second Language / 第二語言"
						description="Shown alongside or instead of English"
					>
							...
							<button
								...
								class={`px-3 py-1.5 rounded-lg text-sm font-medium border transition-colors ${
									locale() === "en"
										...
										: "bg-gray-3 text-gray-12 border-gray-4 hover:bg-gray-4"
								}`}
								onClick={() => setLocale("en")}
							>
								Off
							</button>
							{secondLocaleOptions.map((opt) => (
								<button
									type="button"
									class={`px-3 py-1.5 rounded-lg text-sm font-medium border transition-colors ${
										locale() === opt.value
					...
					<Show when={locale() !== "en"}>
						<SettingItem
							label="Display Mode / 顯示模式"
							description={
								displayMode() === "mixed" ? "Mixed: Edit (編輯)" : "Pure: 編輯"
							}
						>
							<div class="flex gap-2">
								{modeOptions.map((opt) => (
