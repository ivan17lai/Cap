	...
	return (
		<div class="flex flex-col justify-center flex-1 items-center gap-12 p-4 text-[0.875rem] font-normal h-full">
			<Show when={updateError()}>
				<div class="flex flex-col gap-4 items-center text-center max-w-md">
					...
					<p class="text-(--text-tertiary)">
						{t("Please download the latest version manually from cap.so/download. Your data will not be lost.")}
					</p>
			...
			<Show
				...
				fallback={
					!updateError() && (
						<span class="text-(--text-tertiary)">{t("No update available")}</span>
					)
				}
				keyed
			>
							...
							>
								<Match when={updateStatus()?.type === "done"}>
									<div class="flex flex-col gap-4 items-center">
										<p class="text-(--text-tertiary)">
											{t("Update has been installed. Restart Cap to finish updating.")}
										</p>
										<Button onClick={() => relaunch()}>{t("Restart Now")}</Button>
									</div>
								</Match>
								<Match
									when={(() => {
