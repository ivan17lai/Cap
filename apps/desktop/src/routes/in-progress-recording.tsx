	...
	const restartRecording = createMutation(() => ({
		mutationFn: async () => {
			const shouldRestart = await dialog.confirm(
				t("Are you sure you want to restart the recording? The current recording will be discarded."),
				{ title: t("Confirm Restart"), okLabel: t("Restart"), cancelLabel: t("Cancel") },
			);
	...
	const deleteRecording = createMutation(() => ({
		mutationFn: async () => {
			const shouldDelete = await dialog.confirm(
				...
				{ title: t("Confirm Delete"), okLabel: t("Delete"), cancelLabel: t("Cancel") },
			);
								...
								<div class="flex items-center gap-1">
									...
									<Show when={hasCameraInput() && disconnectedInputs.camera}>
										<div
											...
											title={t("Camera disconnected - recording continues without camera overlay")}
										>
